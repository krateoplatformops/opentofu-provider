...
// Package opentofu provides a harness for the OpenTofu CLI. It is known to
// work with OpenTofu v1.6.0-dev.
package opentofu
...
// Error strings.
const (
	errParse        = "cannot parse OpenTofu output"
	errWriteVarFile = "cannot write tfvars file"
	errSemAcquire   = "cannot acquire semaphore for tfinit"

	errFmtInvalidConfig = "invalid OpenTofu configuration: found %d errors"

	tfDefault = "default"
)

const varFilePrefix = "provider-opentofu-"
...
// A Harness for running the opentofu binary.
type Harness struct {
...
// An InitOption affects how a OpenTofu is initialized.
type InitOption func(o *initOptions)

// FromModule can be used to initialize a OpenTofu configuration from a module,
...
// WithInitArgs supplies a list of OpenTofu argument.
...
// Validate a OpenTofu configuration. Note that there may be interplay between
...
// Workspace selects the named OpenTofu workspace. The workspace will be
...
func (h Harness) Workspace(ctx context.Context, name string) error {
	...
	// workspace doesn't exist, which causes OpenTofu to return non-zero. This
...
// DeleteCurrentWorkspace deletes the current OpenTofu workspace if it is not the default.
...
// An OutputType of OpenTofu.
type OutputType int

// OpenTofu output types.
const (
...
// Outputs extracts outputs from OpenTofu state.
...
// Resources returns a list of resources in the OpenTofu state.
...
// An Option affects how a OpenTofu is invoked.
type Option func(o *options)

// WithArgs supplies a list of OpenTofu argument.
...
// WithVar supplies a OpenTofu variable.
...
// The FileFormat of a OpenTofu file.
type FileFormat int

// Supported OpenTofu file formats.
const (
...
// WithVarFile supplies a file of OpenTofu variables.
func WithVarFile(data []byte, f FileFormat) Option {
	return func(o *options) {
		// OpenTofu uses the file suffix to determine file format.
...
// Diff invokes 'opentofu plan' to determine whether there is a diff between
...
func (h Harness) Diff(ctx context.Context, o ...Option) (bool, error) {
	...
	// The -detailed-exitcode flag will make opentofu plan return:
...
// Apply a OpenTofu configuration.
...
// Destroy a OpenTofu configuration.
